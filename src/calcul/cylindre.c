...
/*   Updated: 2023/05/08 18:26:19 by julmuntz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// // 	*ray_origin_projected = subtract_vector(oc, vector_multiply(cylindre->axis, dot(oc, cylindre->axis)));
// // 	*ray_direction_projected = subtract_vector(ray->direction, vector_multiply(cylindre->axis, dot(ray->direction, cylindre->axis)));
// // }
...
// 			subtract_vector(vector_multiply(ray->direction, t_values[0]), oc));
// 	dist[1] = dot(cylindre->axis,
// 			subtract_vector(vector_multiply(ray->direction, t_values[1]), oc));
// }
...
//     t_vector adjusted_center = cylindre->center;
//     adjusted_center.y /= 2; // Adjust the Y origin point

//     oc = subtract_vector(ray->origin, adjusted_center);
//     ray_origin_projected = subtract_vector(oc, vector_multiply(cylindre->axis, dot(oc, cylindre->axis)));
//     ray_direction_projected = subtract_vector(ray->direction, vector_multiply(cylindre->axis, dot(ray->direction, cylindre->axis)));

//     coeffs[0] = dot(ray_direction_projected, ray_direction_projected);
//     coeffs[1] = 2 * dot(ray_origin_projected, ray_direction_projected);
//     coeffs[2] = dot(ray_origin_projected, ray_origin_projected) - pow(cylindre->diameter / 2, 2);
...
//     calculate_intersection_distances(t_values, ray, cylindre, dist);
//     is_valid[0] = (dist[0] >= 0 && dist[0] <= cylindre->height && t_values[0] > EPSILON);
//     is_valid[1] = (dist[1] >= 0 && dist[1] <= cylindre->height && t_values[1] > EPSILON);
...
//     if (is_valid[0] && is_valid[1]) {
//         *distance = fmin(t_values[0], t_values[1]);
//     } else {
//         *distance = is_valid[0] ? t_values[0] : t_values[1];
//     }
		...
		t_vector *ray_origin_projected, t_vector *ray_direction_projected)
{
	...
	*ray_origin_projected = subtract_vector(ray->direction,
			vector_multiply(cylindre->axis, dot(ray->direction,
					cylindre->axis)));
	*ray_direction_projected = subtract_vector(oc,
			vector_multiply(cylindre->axis, dot(oc,
					cylindre->axis)));
}
