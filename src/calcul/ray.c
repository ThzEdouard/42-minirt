...
/*   Updated: 2023/04/12 21:27:34 by eflaquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ray	init_ray(t_coord origin, t_ca cam, int x, int y)
{
	...
	double fov_adjustment = tan((cam.fov * (180 / M_PI) / 2.0));
	double aspect_ratio = (WIDTH) / (HEIGHT);

	result.origin = origin;
	result.diection.x = ((((x + 0.5) / WIDTH) * 2.0 - 1.0) * aspect_ratio) * fov_adjustment;
